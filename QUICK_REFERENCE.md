...
[s3]
multipart_threshold = 10MB   # Files >= this size use multipart upload (default: 8 MiB)
multipart_chunksize = 5MB    # Size of each part (default: 8 MiB)
```

Per-command CLI flags override the config-file defaults:
```bash
...
```
Accepted CLI suffixes: `k`/`K`, `m`/`M`, `g`/`G`, or plain bytes.
...
### Slow uploads
```bash
# Per-command: CLI flags override config defaults
